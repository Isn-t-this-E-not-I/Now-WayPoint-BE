...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        if(userDetail.isFirstLogin()){
            response.sendRedirect("https://www.now-waypoint.com/oauth2/redirect?token=" + accessToken + "&nickname=" + nickname + "&isFirstLogin=true");
        } else {
            response.sendRedirect("https://www.now-waypoint.com/oauth2/redirect?token=" + accessToken + "&nickname=" + nickname + "&isFirstLogin=false");
        }
